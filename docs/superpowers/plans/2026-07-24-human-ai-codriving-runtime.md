...
2. 用户与 Codex 默认共享同一个先进先出命令队列，已获授权的命令按进入队列的先后串行执行，普通用户输入不改变 Codex 接入状态；待审批动作只冻结后续 Codex，不阻塞用户终端；开启“完全接管”后阻止未启动的 Codex 动作，正在运行的前台命令必须由用户明确中断，不能静默抢占。
...
- 同一共享终端同一时刻只有一个前台动作；
- 用户和 Codex 已获授权的命令进入同一个先进先出队列，普通用户输入不自动暂停 Codex；
- 排队动作显示为“排队中”，轮到队首并即将写入 SSH 时才显示为“进行中”；
