...
1. “每次询问 / 自动只读 / 信任会话”是三档 Codex 授权等级；“用户接管 / Codex 暂停”是另一条状态轴，不增加第四档。
2. 用户输入优先只表示暂停尚未执行的 Codex 动作；正在运行的前台命令必须由用户明确中断，不能静默抢占。
3. 原始交互 PTY 无法可靠预判所有回车。强制数据护栏对双方生效，结构化 Codex 命令始终预检，用户高危命令则在受保护命令模式预检；进入原始 TTY 时必须明确提示并暂停 Codex。
...
- `pauseCodex / resumeCodex`：用户接管和恢复；
